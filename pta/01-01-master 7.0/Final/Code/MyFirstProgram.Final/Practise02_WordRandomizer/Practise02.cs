...
namespace MyFirstProgram.Final.Practise02_WordRandomizer;

public class Practise02 : Exercise0101Base
{
    public override void Run()
    {
        ...
        */
        ...
        WriteWords("Enter comma separated words:");
        WriteSpace();

        string[] words = ReadStringInput().Split(",", StringSplitOptions.TrimEntries);

        WriteWords("Starting word randomizer, hit key [enter]...");
        for (int i = 0; i < GetRandomWordWriteCount(); i++)
        {
            ReadStringInput();
            WriteWord(GetRandomWord(words));
        }
        WriteNewLine();
